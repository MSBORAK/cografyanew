...
const EuropeMap = ({ onBackToMenu }) => {
  ...
  const quizCountries = worldPaths.filter(c => europeCountries.includes(c.id));
    ...
    >
      <View style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerLeft}>
            <Text style={styles.title}>Avrupa Quiz</Text>
            {!isCompleted ? (
      ...
      <View style={styles.mapContainer}>
        <Animated.View style={[styles.mapWrapper, { transform: [{ scale }, { translateX }, { translateY }] }]} {...panResponder.panHandlers}>
          <Svg width={MAP_WIDTH} height={MAP_WIDTH * 0.507} viewBox={continentViewBox.europe.viewBox} preserveAspectRatio={continentViewBox.europe.preserveAspectRatio} style={styles.svg}>
            ...
            <G>
              {worldPaths.map((country, index) => {
                ...
                let fillColor = isEurope ? getCountryColor(index) : '#E5E7EB';
                let strokeColor = '#FFFFFF';
                let opacity = isEurope ? 0.9 : 0.3;
                ...
                return (
                  <G key={country.id} onPress={() => isEurope && handleCountryPress(country)} onPressIn={() => isEurope && handleCountryPress(country)}>
...
const styles = StyleSheet.create({
  ...
  resetButton: { backgroundColor: '#10B981', paddingVertical: 8, paddingHorizontal: 20, borderRadius: 8, alignItems: 'center' },
  resetButtonText: { fontSize: 13, fontWeight: '600', color: '#FFFFFF' },
  zoomResetButton: { position: 'absolute', bottom: 16, right: 16, width: 44, height: 44, borderRadius: 22, backgroundColor: '#10B981', justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5 },
});
